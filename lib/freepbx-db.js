...
async function createOrUpdateRecording(name, displayName) {
  ...
  try {
    ...
    if (existing.length > 0) {
      ...
      await conn.execute(
        'UPDATE recordings SET displayname = ?, description = ? WHERE id = ?',
        [displayName, `Custom recording: ${displayName}`, recordingId]
      );
    } else {
      const [result] = await conn.execute(
        'INSERT INTO recordings (displayname, filename, description) VALUES (?, ?, ?)',
        [displayName, filePath, `Custom recording: ${displayName}`]
      );
...
// --- Ring Group + Time Condition destinations ---

async function setRinggroupPostdest(grpnum, dest) {
  await pool.execute(
    'UPDATE ringgroups SET postdest = ? WHERE grpnum = ?',
    [dest, String(grpnum)]
  );
}

async function setTimeconditionFalsegoto(tcId, dest) {
  await pool.execute(
    'UPDATE timeconditions SET falsegoto = ? WHERE timeconditions_id = ?',
    [dest, tcId]
  );
}
...
module.exports = {
  ...
  setRinggroupPostdest,
  setTimeconditionFalsegoto,
};

...
async function createTimecondition(conn, storeSlug, timegroupId, closedAnnouncementId) {
  ...
  const [result] = await conn.execute(
    'INSERT INTO timeconditions (displayname, time, timezone, truegoto, falsegoto, mode, generate_hint) VALUES (?, ?, ?, ?, ?, ?, ?)',
...
async function createDaynight(conn, ext, storeName, timeconditionId, ringGroupNum) {
  ...
  await conn.execute('INSERT INTO daynight (ext, dmode, dest) VALUES (?, ?, ?)',
    [e, 'day', `timeconditions,${timeconditionId},1`]);

  // NIGHT mode → ring group
  await conn.execute('INSERT INTO daynight (ext, dmode, dest) VALUES (?, ?, ?)',
    [e, 'night', `ext-group,${ringGroupNum},1`]);

  // Metadata rows (required by FreePBX module)
  await conn.execute('INSERT INTO daynight (ext, dmode, dest) VALUES (?, ?, ?)',
    [e, 'fc_description', `${storeName} override`]);
  await conn.execute('INSERT INTO daynight (ext, dmode, dest) VALUES (?, ?, ?)',
    [e, 'day_recording_id', '0']);
  await conn.execute('INSERT INTO daynight (ext, dmode, dest) VALUES (?, ?, ?)',
    [e, 'night_recording_id', '0']);

  // Feature code: *{ext} toggles this call flow control
  ...
  await conn.execute(
    ...
    ['daynight', `toggle-mode-${ext}`, `${ext}: ${storeName} override`, '', defaultCode, customCode]
  );
}

async function createInboundRoute(conn, did, storeName, callflowExt) {
  await conn.execute(
    'INSERT INTO incoming (cidnum, extension, destination, privacyman, alertinfo, ringing, mohclass, description, grppre, delay_answer, pricid) VALUES (?, ?, ?, 0, ?, ?, ?, ?, ?, 0, ?)',
